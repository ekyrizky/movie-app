...
void main() {
  ...
  blocTest<MovieRecommendationsBloc, MovieRecommendationsState>(
    ...
    build: () {
      when(usecase.execute(testId)).thenAnswer((_) async => Right(testMovieList));
      return movieRecommendationsBloc;
    },
    act: (bloc) => bloc.add(FetchMovieRecommendations(testId)),
    expect: () => [
    ...
    verify: (bloc) {
      ...
      return FetchMovieRecommendations(testId).props;
    },
  );
