...
void main() {
  ...
  testWidgets('All required widget should display', (WidgetTester tester) async {
    ...
    when(() => fakeMovieRecommendationsBloc.state).thenReturn(MovieRecommendationsHasData(testMovieList));
  ...
  testWidgets('Watchlist button should display add icon when movie not added to watchlist',
    ...
    when(() => fakeMovieDetailBloc.state).thenReturn(MovieDetailHasData(testMovieDetail));
    when(() => fakeWatchlistMoviesBloc.state).thenReturn(MovieIsAddedToWatchlist(false));
    when(() => fakeMovieRecommendationsBloc.state).thenReturn(MovieRecommendationsHasData(testMovieList));
  ...
  testWidgets('Watchlist button should display check icon when movie is added to watchlist',
      (WidgetTester tester) async {
    when(() => fakeMovieDetailBloc.state).thenReturn(MovieDetailHasData(testMovieDetail));
    when(() => fakeWatchlistMoviesBloc.state).thenReturn(MovieIsAddedToWatchlist(true));
    when(() => fakeMovieRecommendationsBloc.state).thenReturn(MovieRecommendationsHasData(testMovieList));
