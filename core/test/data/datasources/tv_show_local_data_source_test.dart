...
void main() {
  ...
  group('save watchlist', () {
    test('should return success message when insert to database is success', () async {
      // arrange
      when(mockDatabaseHelper.insertTVShowWatchlist(testTVShowTable)).thenAnswer((_) async => 1);
      // act
      final result = await dataSource.insertWatchlist(testTVShowTable);
      // assert
      expect(result, watchlistAddMessage);
    });

    test('should throw DatabaseException when insert to database is failed', () async {
      // arrange
      when(mockDatabaseHelper.insertTVShowWatchlist(testTVShowTable)).thenThrow(Exception());
      // act
  ...
  group('remove watchlist', () {
    test('should return success message when remove from database is success', () async {
      // arrange
      when(mockDatabaseHelper.removeTVShowWatchlist(testTVShowTable)).thenAnswer((_) async => 1);
      // act
      final result = await dataSource.removeWatchlist(testTVShowTable);
      // assert
      expect(result, watchlistRemoveMessage);
    });

    test('should throw DatabaseException when remove from database is failed', () async {
      // arrange
      when(mockDatabaseHelper.removeTVShowWatchlist(testTVShowTable)).thenThrow(Exception());
      // act
  ...
  group('Get TV Show Detail By Id', () {
    ...
    test('should return TV Show Detail Table when data is found', () async {
      // arrange
      when(mockDatabaseHelper.getTVShowById(tId)).thenAnswer((_) async => testTVShowTable.toJson());
      // act
  ...
  group('get watchlist tv shows', () {
    test('should return list of TVShowTable from database', () async {
      // arrange
      when(mockDatabaseHelper.getWatchlistTVShows()).thenAnswer((_) async => [testTVShowTable.toJson()]);
      // act
