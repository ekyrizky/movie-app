...
void main() {
  ...
  blocTest<TVShowRecommendationsBloc, TVShowRecommendationsState>(
    ...
    build: () {
      when(usecase.execute(testId)).thenAnswer((_) async => Right(testTVShowList));
      return tvShowRecommendationsBloc;
    },
    act: (bloc) => bloc.add(FetchTVShowRecommendations(testId)),
    expect: () => [
    ...
    verify: (bloc) {
      ...
      return FetchTVShowRecommendations(testId).props;
    },
  );
