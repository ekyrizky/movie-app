...
void main() {
  ...
  blocTest<SearchMoviesBloc, SearchMoviesState>(
    ...
    build: () {
      when(mockSearchMovies.execute(testQuery)).thenAnswer((_) async => Right(testMovieList));
  ...
  blocTest<SearchMoviesBloc, SearchMoviesState>(
    ...
    build: () {
      when(mockSearchMovies.execute(testQuery)).thenAnswer((_) async => const Left(ServerFailure('Server Failure')));
      return searchMoviesBloc;
    },
    act: (bloc) => bloc.add(OnQueryMoviesChange(testQuery)),
    wait: const Duration(milliseconds: 200),
  ...
  blocTest<SearchMoviesBloc, SearchMoviesState>(
    ...
    build: () {
      when(mockSearchMovies.execute(testQuery)).thenAnswer((_) async => const Right([]));
      return searchMoviesBloc;
    },
    act: (bloc) => bloc.add(OnQueryMoviesChange(testQuery)),
    wait: const Duration(milliseconds: 200),
